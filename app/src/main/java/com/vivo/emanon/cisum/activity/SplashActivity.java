...
public class SplashActivity extends BaseActivity {
    ...
    private boolean checkService() {
        ...
        if (AppCache.getAudioService() == null) {
            ...
            new Handler().postDelayed(this::bindAudioService, 100);
        }
        if (AppCache.getScreenLockService() == null) {
            startScreenLockService();
            isService = false;

            new Handler().postDelayed(this::bindScreenLockService, 100);
        }
        if (AppCache.getStepCounterService() == null) {
            startStepCounterService();
            isService = false;

            new Handler().postDelayed(this::bindStepCounterService, 100);
        }
        return isService;
    }
    ...
    private static class ScreenLockServiceConnection implements ServiceConnection {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
    ...
    private static class StepCounterServiceConnection implements ServiceConnection {
        @Override
        public void onServiceConnected(ComponentName name, IBinder service) {
    ...
    private void loadSplash() {
        ...
        HttpUtil.sendOkHttpRequest(requestSplash, new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException, NullPointerException {
                final String splash = Objects.requireNonNull(response.body()).string();
                ...
                editor.apply();
                runOnUiThread(() -> Glide.with(SplashActivity.this).load(splash).into(ivSplash));
            }
        });
    }
